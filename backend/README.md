...
- JWT token-based API security using Firebase ID tokens via Axios interceptors (no race conditions).
...
- Coupon/promo code support with percentage and fixed discount calculations applied directly to Stripe sessions.
...
- Custom cover photo upload with an interactive **vertical repositioning slider** (avoids heads getting cut off).
- Profile avatar, name, role badge, and registration date displayed in a premium card layout.
- Auto-prefill shipping name, address, and phone number from the saved user profile on checkout.
...
✅ **Analytics Dashboard**
- Admin statistics page with an animated **Recharts AreaChart** showing platform-wide sales and order trends with tooltips and responsive grids.
...
### 👑 Admin
- **Manage Users:** View all registered users with live search, role/status filters, and summary metric cards (Total, Admins, Sellers, Pending).
...
- **Safe Deletion:** Custom confirmation modal displays the target account email before permanently deleting a user.
...
- **Seller Verification:** Request verification status via the profile dashboard.
...
| imgbb (via API) | Cloud Image Hosting for plants & cover photos |
